...
attributePortfoliosWe <- function(stoxwe){
  ...
  pfs = stoxwe_add %>%
    ...
    mutate(kkr_3tile_inner = if_else(topic_kk <= quantile(topic_kk, 0.9, na.rm = TRUE), 1, 2)) %>%
    ungroup() %>%
...
create_eret_we_panel <- function(stoxwe_orig, cequity_mapper, topic_map, ffmat, pfn){
  ...
  kkrhml_ret = stoxwe_with_pfs %>%
    ...
    transmute(yw, kkrhml = get(paste0("kk", quantiles)) - get(paste0("kk1")))
  ...
  #   transmute(ym, kkrhml = get(paste0("kk", quantiles)) - get(paste0("kk1")))
...
understand_topics <- function(topic_map_labeled, labels, quantiles, textfolder){
  ...
  topic_map <- topic_map_labeled %>%
    group_by(year) %>%
    mutate(topic_kk_90th_percentile = quantile(topic_kk, 0.9, na.rm = TRUE)) %>%
    mutate(ntile_topic_kk = if_else(topic_kk <= topic_kk_90th_percentile, 1, 2)) %>%
    mutate(kkpt_ntile = ntile(K_int_Know/at, nt)) %>%
    mutate(ikpt_ntile = ntile(K_int/at, nt)) %>%
    ungroup() %>%
    select(-topic_kk_90th_percentile)
