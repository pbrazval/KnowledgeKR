...
class Stargazer:
    ...
    def show_t_statistic(self, show):
        assert type(show) == bool, 'Please input True/False'
        print("Set show_t_statistic")
        self.show_t_statistic = show
...
class LaTeXRenderer(Renderer):
    ...
    def generate_cov_precision(self, cov_name):
        ...
        for md in self.model_data:
            if cov_name in md['cov_names']:
                cov_text += '& ('
                if self.show_t_statistic:
                    cov_text += self._float_format(md['t_values'][cov_name])
